...
const defVal: LoginForm = {
    email: "",
    password: "",
}

function LoginPage() {
    const {handleSubmit, register} = useForm<LoginForm>({defaultValues: defVal});
    ...
    const submit = async (v: LoginForm) => {
        setLoading(true)
        try {
            const loginResult = await AuthService.login(v);
            if (loginResult){
                navigate({ to: '/admin' });
            } else {
                setError(true);
            ...
            toast.error(`Fehler beim Einloggen: ${error.message}`)
        } finally {
            setLoading(false)
        }
    }
                ...
                <div className="">
                    <div className="mb-2">
                        ...
                        <Input
                            ...
                            name="email"
                            id="email"
                            placeholder="admin@container.de"
                        />
                    </div>
                    ...
                    {error && (<div
                        ...
                        role="alert">
                        <span className="font-medium">Fehler beim Einloggen</span>
                    </div>)}
                </div>
            </div>
        </form>
